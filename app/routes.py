from flask import Blueprint, render_template, request, redirect, session, make_response, send_file, url_for
import sqlite3
import pandas as pd
...
from app.funct import get_connection, login_required, Calculator

routes = Blueprint('routes', __name__, template_folder=os.path.join(os.path.dirname(__file__), '..', 'templates'))
...
@routes.route('/adauga', methods=['GET', 'POST'])
@login_required
def adauga_angajat():
    if request.method == 'POST':
        try:
...
@routes.route('/afisare', methods=['GET', 'POST'])
@login_required
...
# === Sterge angajat ===
@routes.route('/sterge/<cnp>', methods=['GET', 'POST'])
@login_required
...
# === Modificare angajat ===
@routes.route('/modifica/<cnp>', methods=['GET', 'POST'])
@login_required
...
# === CALCUL SALAR ===
@routes.route('/calcul_salar/<cnp>', methods=['GET', 'POST'])
@login_required
...
@routes.route('/fluturas_salar/<cnp>/<an>/<luna>')
@login_required
...
@routes.route('/fluturas_pdf/<cnp>/<an>/<luna>')
@login_required
...
def export_excel():
    ...
    file_path = os.path.join(BASE_DIR, "angajati_export.xlsx")
